...
async def main():
    ...
    being_input = TextInputVisualizer(
        TextInputManager(
            validator=lambda input: (
                len(input) <= 2 and input in [str(num_) for num_ in range(1, 100)] + [""]
            )
        )
    )
    ...
    seed_input = TextInputVisualizer(
        TextInputManager(
            validator=lambda input: (
                len(input) <= 5 and input in [str(num_) for num_ in range(0, 10000)] + [""]
            )
        )
    )
    ...
    hunter_input = TextInputVisualizer(
        TextInputManager(
            validator=lambda input: (
                len(input) <= 2 and input in [str(num_) for num_ in range(1, 100)] + [""]
            )
    ...
    food_input = TextInputVisualizer(
        TextInputManager(
            validator=lambda input: (
                len(input) <= 2 and input in [str(num_) for num_ in range(1, 100)] + [""]
            )
    ...
    inputs = [
        seed_input.surface.get_rect(topleft=[center_pos(title)[0] + 20, center_pos(title)[1] / 0.89]),
        being_input.surface.get_rect(topleft=[center_pos(title)[0] + 20, center_pos(title)[1] / 0.75]),
        hunter_input.surface.get_rect(
            topleft=[center_pos(title)[0] + 20, (center_pos(title)[1] / 0.75) + 100]
        ),
        food_input.surface.get_rect(
            topleft=[center_pos(title)[0] + 20, (center_pos(title)[1] / 0.75) + 200]
        ),
    ]
    ...
    buttons = [
        Button(
            [center_pos(title)[0] + 20, center_pos(title)[1] / 1.75],
            [
                scale_image(pygame.image.load("start_unpressed.png")),
                scale_image(pygame.image.load("start_pressed.png")),
            ],
            [play, []],
        ),
        Button(
            [center_pos(title)[0] + 20, (center_pos(title)[1] / 1.75) + 100],
            [
                scale_image(pygame.image.load("start_unpressed.png")),
                scale_image(pygame.image.load("start_pressed.png")),
            ],
    ...
    death_button = Button(
        [center_pos(title)[0] + 20, center_pos(title)[1] * 1.25],
        [
            scale_image(pygame.image.load("start_unpressed.png")),
            scale_image(pygame.image.load("start_pressed.png")),
        ],
    ...
    back_button = Button(
        [center_pos(title)[0] + 20, center_pos(title)[1] * 1.5],
        [
            scale_image(pygame.image.load("start_unpressed.png")),
            scale_image(pygame.image.load("start_pressed.png")),
        ],
    ...
    while run:
        ...
        if screen == 2:
            ...
            if ecosystem.is_interactive:
                ...
                if len(player) == 0:
                    ...
                    win.blit(
                        death,
                        [
                            death_button.pos[0] + 35,
                            death_button.pos[1] + 13 + (8 * death_button.current),
                        ],
                    )
                    back_button.update()
                    win.blit(
                        back_,
                        [back_button.pos[0] + 50, back_button.pos[1] + 13 + (8 * back_button.current)],
                    )
        else:
            win.blit(bg, (0, 0))
            ...
            win.blit(
                hunter_input.surface, [center_pos(title)[0] + 20, (center_pos(title)[1] / 0.75) + 100]
            )
            if hunter_input.value == "":
                win.blit(h, [center_pos(title)[0] + 20, (center_pos(title)[1] / 0.75) + 100])
            win.blit(
                food_input.surface, [center_pos(title)[0] + 20, (center_pos(title)[1] / 0.75) + 200]
            )
            if food_input.value == "":
